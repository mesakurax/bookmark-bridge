...
一个简洁的 Windows 本地同步工具，在不修改 Chrome Sync / Edge Sync 设置的前提下，手动桥接收藏夹、密码和浏览记录。

## 扩展弹窗（推荐）

日常使用无需打开终端。在 Chrome 或 Edge 工具栏点击 Bookmark Bridge，即可直接运行 7 个主要操作。同步时即使浏览器自动重启，本地后台任务仍会继续；重新打开弹窗可恢复进度和日志。

密码迁移中的“已导出”和“已导入”也直接在弹窗中确认。命令行仍作为本地后端和高级/排错入口保留。
...
Chromium 可能不提供“移动收藏夹”特殊根目录。此时工具会在账户的“其他收藏夹”中创建 `移动收藏夹（Bookmark Bridge）`，保留原有分类；重复同步不会再创建第二个。如果以后浏览器提供了真正的移动根目录，内容会自动迁回。
请不要手动改名这个包装目录，否则工具会将它当成普通收藏夹保留。
...
导出、Windows Hello 和导入确认必须由你亲自操作，因此密码迁移不是无人值守流程。扩展弹窗会在两个步骤停下等待确认；临时明文 CSV 在确认导入后删除。目标浏览器遇到相同 `网址 + 用户名` 时会显示冲突；本次源浏览器是主导时选择“替换”，否则选择“跳过”。
...
安装程序已经注册仅限当前用户的 Native Messaging 主机。扩展只接受 7 个固定同步操作，并使用一次性随机任务令牌；没有常驻服务，收藏夹内容不会发送到网络。
